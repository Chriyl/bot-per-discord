...
client = commands.Bot(command_prefix="!", intents=intents) # specifici l'intenti del bot


@client.event # questo é il messaggio che esce in console se il bot va online
async def on_ready():
    print(f'Bot is ready. Logged in as {client.user.name}')

@client.command() # questo é il ping del bot
async def ping(ctx): # ctx é l'argomento di colui che ha mandato il messaggio ed é un oggetto quindi ha metodi e elementi
    await ctx.send("pong")

client.run(TOKEN) # qui il bot runna